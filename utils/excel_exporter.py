    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        for i, r in enumerate(display_results):
            summary_data.append({
                ...
                'Sweet Zone Quality': r.get('sweet_zone_quality', 'Good'),
                'Current Price': f"₹{r['current_price']:.2f}",
                'P5 (Entry)': f"₹{r['p5']:.2f}",
        ...
        if ranked_bullish:
            ...
            for r in ranked_bullish[:15]:
                bullish_data.append({
                    ...
                    'EPA Target': f"₹{r['epa']:.2f}",
                    'Stop Loss': f"₹{r['stop_loss']:.2f}",
                    '1-3 Line': r.get('line_13_quality', r.get('sweet_zone_quality', 'Good')),
                    '2-4 Line': r.get('line_24_quality', 'Good'),
                    'Convergence': 'Yes' if r.get('converging', True) else 'No',
                    'Entry Quality': r.get('sweet_zone_quality', 'Good'),
                })
        ...
        for r in display_results[:10]:
            detail_data.append({
                ...
                'EPA': f"₹{r['epa']:.2f}",
                'Sweet Zone Quality': r.get('sweet_zone_quality', 'Good'),
                'Line 1-3': r.get('line_13_quality', r.get('sweet_zone_quality', 'Good')),
                'Line 2-4': r.get('line_24_quality', 'Good'),
                'Sector': r['sector'],
                'RSI': f"{r['rsi']:.1f}",
